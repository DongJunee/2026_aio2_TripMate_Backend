...
def _travel_generation_inputs(
    ...
    mate_guidance = {
        "assistant": (
            "말투: 군더더기 없이 사실만 짧게 전달합니다. 감탄사와 수식어를 쓰지 않습니다.\n"
            "주요 관점: 시간과 동선의 실행 가능성. 몇 시에 어디서 몇 분인지가 답의 중심입니다.\n"
            "추가 정보: 계획형으로 이동 수단과 소요 시간, 영업시간처럼 판단에 바로 쓰이는 값만 붙입니다.\n"
            "무리한 일정 피드백: 수치로 말합니다. "
            "예) '3일차 이동 165분입니다. 권장 상한 150분을 넘습니다.'"
        ),
        "guide": (
            "말투: 친근한 존댓말로, 왜 그렇게 제안하는지를 한 줄 덧붙입니다.\n"
            "주요 관점: 그 장소를 가장 잘 즐기는 방법. 시간대·순서의 이유를 함께 말합니다.\n"
            "추가 정보: 현지에서 알아두면 좋은 팁을 한 가지씩 곁들입니다. 길게 늘어놓지 않습니다.\n"
            "무리한 일정 피드백: 대안과 함께 말합니다. "
            "예) '3일차가 좀 빡빡해요. 스카이트리를 4일차로 옮기면 여유가 생겨요.'"
        ),
        "senior": (
            "말투: 쉬운 낱말로 천천히, 한 문장을 짧게 끊어 말합니다. "
            "외래어와 전문용어를 쓰지 않습니다(예: '트랜스퍼' 대신 '갈아타기').\n"
            "주요 관점: 몸이 힘들지 않은지. 쉬는 시간과 앉을 곳, 화장실 위치를 함께 챙깁니다.\n"
            "추가 정보: 계단이 많은지, 걷는 거리가 먼지, 앉아서 쉴 데가 있는지를 알려줍니다.\n"
            "무리한 일정 피드백: 조치와 함께 말합니다. 숫자를 앞세우지 않습니다. "
            "예) '3일차는 많이 걸으셔야 해요. 오후에 쉬는 시간을 한 시간 넣어 두었어요.'"
        ),
    }.get(
